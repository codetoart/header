...
class Header {
  ...
  constructor({ data, config, api, readOnly }) {
    ...
    /**
     * wDiv: parent wrapper of all
     */
    this.wDiv = this.getUI();
    this.anchorEditorWrapper = this.getAnchorEditorWrapper();
  }
  ...
  getWdiv() {
    ...
    //wDiv.appendChild(this._element);
    let newTag = this.getTag();
    this.tag = newTag;
    wDiv.appendChild(newTag);

    //add editor
    if (this._showAnchorEditor) {
      this.anchorEditorWrapper = this.getAnchorEditorWrapper();
      wDiv.appendChild(this.anchorEditorWrapper);
    }

    //not working with this
    //if(this.wDiv)this.wDiv.parentNode.replaceChild(wDiv, this.wDiv);

    return wDiv;
  }

  getAnchorEditorWrapper() {
    //anchor editor
    const anchorEditorWrapper = document.createElement('div');
    anchorEditorWrapper.classList.add('anchorEditorWrapper');
    anchorEditorWrapper.style.top = this.getAnchorEditorWrapperPositionTop();
    if (this._showAnchorEditor) {//dont return
      anchorEditorWrapper.classList.add('show');

      //input 
      const newAnchorInputField = this.getAnchorInputField();
      this.anchorInputField = newAnchorInputField;
      anchorEditorWrapper.appendChild(this.anchorInputField);

      //save button
      const newSaveAnchorButton = this.getSaveAnchorButton();
      anchorEditorWrapper.appendChild(newSaveAnchorButton);
    } else {//return empty
      anchorEditorWrapper.innerHTML = '';
      return anchorEditorWrapper;
    }
  ...
  getAnchorInputField() {
    ...
    const anchorInputField = document.createElement('div');
    anchorInputField.setAttribute("contenteditable", (this.readOnly) ? false : true);
    anchorInputField.placeholder = "Add anchor";
    // this.anchorInputField.value = this._anchor;
    anchorInputField.innerHTML = this._anchor;
    anchorInputField.classList.add("anchorInputField");

    //on focus out event save and exit
    anchorInputField.addEventListener('focusout', (e) => {
      this.linkAnchorToDiv();
    });

    //escape and enter event handling
    anchorInputField.addEventListener('keydown', (e) => {
    ...
    return anchorInputField;
  }

  getSaveAnchorButton() {
    //button for save and hide: anchor editor
    const saveAnchorButton = document.createElement('div');
    saveAnchorButton.innerHTML = require('./../assets/check.svg').default;
    saveAnchorButton.classList.add('saveAnchorButton');
    saveAnchorButton.title = "Save";
    saveAnchorButton.addEventListener('click', (e) => {
      this.linkAnchorToDiv();
    });
    return saveAnchorButton;
  }
   ...
   */
  linkAnchorToDiv() {
    this._anchor = this.anchorInputField.innerHTML;
    console.log('this._anchor: ', this._anchor);
    this.tag.id = this._anchor;
    this.hideAnchorEditor(this._anchor);
  }

  /**
   * set anchor editor position depend on header level
   */
  getAnchorEditorWrapperPositionTop() {
  ...
  renderSettings() {
    ...
    this.levels.forEach(level => {
      ...
      selectTypeButton.addEventListener('click', () => {
        ...
        //reset anchor editor top position
        if (this._showAnchorEditor) {
          this.anchorEditorWrapper.style.top = this.getAnchorEditorWrapperPositionTop();
        }
      });
  ...
  showAnchorEditor() {
    ...
    this.anchorInputField.innerHTML = this._anchor;//reset input value to anchor
  ...
  save(toolsContent) {
    return {
      text: this.tag.innerHTML,//toolsContent.innerHTML,
